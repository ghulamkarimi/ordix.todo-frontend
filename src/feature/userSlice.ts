...
import { getCurrentUser, getUsers, requestPasswordReset, userLogin, userLogout, userRegister, verifyResetCode, resetPassword } from "@/service";
...
export const userRegisterApi = createAsyncThunk(
    ...
    async (user: TUser, { rejectWithValue }) => {
        try {
            ...
            return rejectWithValue(error.response?.data?.error || "Unbekannter Fehler");
        }
    }
);
...
export const verifyResetCodeApi = createAsyncThunk(
    ...
    async ({ email, code }: { email: string; code: string }, { rejectWithValue }) => {
        try {
            ...
            return rejectWithValue(error.response?.data?.error || "Fehler beim Überprüfen des Codes.");
        }
    }
);
...
export const resetPasswordApi = createAsyncThunk(
    ...
    async ({ email, code, newPassword }: { email: string; code: string; newPassword: string }, { rejectWithValue }) => {
        try {
            ...
            return rejectWithValue(error.response?.data?.error || "Fehler beim Zurücksetzen des Passworts.");
        }
    }
);
...
const initialState: IUserState & EntityState<IUser, number> = userAdapter.getInitialState({
    status: 'idle',
    error: null,
    user: null,
});
